...
public class BinaryTree {
	...
	public ArrayList<Node> nodes(){
		...
		PreorderAdd(lst, this.getRoot());
		
		return lst;
	}
	
	public void PreorderAdd(ArrayList<Node> t, Node v){
		t.add(v);
		if(hasLeft(v)) PreorderAdd(t, v.getLeft());
		if(hasRight(v)) PreorderAdd(t, v.getRight());
	}
	
	public int height1(BinaryTree t){
		ArrayList<Node> lst = this.nodes();
		int h = 0;
	...
	public int height2(Node v){
		if(this.isExternal(v)) return 0;
		int h = 0;
		h = Math.max(height2(v.getLeft()), height2(v.getRight()));
		return 1 + h;
	}
